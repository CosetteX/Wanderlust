...
public class MapView extends View {
    ...
    private Thread loadThread = new Thread(new Runnable() {
        ...
        public void run() {
            //定义一个输入流对象去加载xml文件
            //InputStream inputStream = context.getResources().getDrawable(R.drawable.chinalow);
            ...
            try {
                ...
                list.get(32).setName("Yunnan");
                list.get(33).setName("Zhejiang");
                //封装地图的矩形
    ...
    private void handlerTouch(float x, float y) {
        ...
        for (ProvinceItem proviceItem : itemList) {
            ...
            if(proviceItem.isTouch(x/scale,y/scale)){
                ...
                if(name.equals("Zhejiang")){
                    controller.renewAdapter(name);
                }
                else{
                    controller.renewAdapter("Yunnan");
                }
            }
        }
        if(selectItem !=null){
